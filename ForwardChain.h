...
class ForwardChain
{
	...
	user_input_fn _fn;
public:
	ForwardChain(user_input_fn fn) :_fn(fn) {}
	void build_forward_KB(const char* kb_file_name)
	{
		//Read KB file
		jxx::Object json;
		ifstream kbfile;
	...
	void start_fc(const std::vector<variable>& vars)
	{
	...
	variable& get_conclusion() { 
		if (_conclusion_list.size() > 0)
			return _conclusion_list[0];
		else
			return variable();
	}
};
